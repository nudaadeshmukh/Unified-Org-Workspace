...
## Phase 6 — Security Hardening & Automated Tests (2026-07-30)

**Status:** complete

**Ambiguity flagged before writing any code, per the standing "flag before coding" rule:** no Jest/Supertest infrastructure existed anywhere in the repo (no config, no dependency), and all 5 required suites are inherently cross-service (tenant-isolation/sharing need identity+ticket+pr running together; ai-leakage needs audit-service with a mocked Groq call; audit-permissions needs a live `audit_writer` Postgres connection) — none of them cleanly belongs to one service's package the way CLAUDE.md's original Commands section implied. Three decisions were needed before any test code could be written; all three resolved with the user:
1. **How to get testable app instances** — none of the 4 services' `server.js` exported their Express `app`; each unconditionally called `app.listen(...)`. Resolved: guard `app.listen` (and, for audit-service, `startDigestScheduler()`) behind `if (require.main === module)`, and always `module.exports = app`. No behavior change for real `npm run dev` usage — this only matters when a file is `require()`'d instead of executed directly.
2. **Where the test files live** — resolved: a root-level `tests/` directory with its own `tests/helpers/`, not split across each service's package. CLAUDE.md's Commands section was corrected (by the user, in-session) to describe this accurately instead of implying per-service suites that never existed.
3. **Test data isolation** — resolved: a fully separate `froncort_test` Postgres database (same instance, same credentials, all 4 schemas migrated in), never the seeded `froncort` dev database. Each suite creates its own fixtures directly (via the real API, plus direct Prisma access only for things the API can't produce, like backdating a ticket's `createdAt`) and the whole database is disposable — no seed-state pollution risk between phases' manual verification and automated tests ever again.

**1. Security sweep, all 4 services:**
- **Helmet** — already `app.use(helmet())` with defaults in all 4 services since Phase 1; confirmed still correctly positioned (before routes) in every `server.js`. No change needed — helmet's defaults already set the standard hardening headers.
- **CORS allowlist** — replaced the bare `cors()` (an explicit known-TODO comment since Phase 1) in all 4 services with `cors(buildCorsOptions())`. New `packages/shared/corsOptions.js`: parses `CORS_ALLOWED_ORIGINS`, allows exactly those origins (`credentials: true`, required since identity-service's refresh token is an httpOnly cookie the frontend must send/receive cross-origin), and allows requests with no `Origin` header at all (curl, server-to-server — never a browser CORS scenario, so this doesn't weaken real enforcement). Verified live: `Origin: http://localhost:3000` → `Access-Control-Allow-Origin` echoed back; `Origin: http://evil.example.com` → no CORS headers at all (browser would block it).
- **Zod validation on every route body** — grepped every `req.body` reference across all 4 services' route files (`grep -n "req\.body" */src/routes/*.js`). Every single hit is either inside a `.parse(req.body)` call or the login rate-limiter's `keyGenerator` (which only reads `req.body.email` as an optional throttle key, never writes it anywhere) — **no gaps found**, this was already fully covered from Phases 2-5.
- **orgId-filter grep — "the single most valuable check in this whole phase," per the user's own framing.** Grepped every `findUnique`/`findFirst`/`update`/`delete` call across `ticket-service`, `pr-service`, and `audit-service`'s services, then pulled the surrounding context for every bare `where: { id: ... }` lookup. **Every one is immediately followed by an explicit `orgScope.ownsResource(...)` check, an `assertOwnTicket`/`assertOwnPR` helper, or an own-`userId` check (notifications)** — the two exceptions that don't show a check on the very next line (`resolveTicketAccess`/`resolvePRAccess`) are the resolver functions themselves, which perform the ownership/share check as their entire purpose. **No gaps found** — the discipline from Phases 3/4/5 held up under a full re-audit.
- **Refresh-token rotation + reuse detection, re-verified live with all 4 services running concurrently** (not just identity-service in isolation, as Phase 2's original manual proof was): login → refresh (rotate #1) → refresh again (rotate #2, using rotate #1's cookie) → replay rotate #1's now-superseded cookie → `401 SESSION_REVOKED` → the legitimate rotate #2 cookie is ALSO now dead → `401 UNAUTHENTICATED`. Identical to Phase 2's originally-proven behavior — confirms ticket/pr/audit services being alive alongside identity-service introduced no cross-service session-state confusion (expected, since only identity-service ever touches Redis session state, but confirmed rather than assumed).

**2. Test infrastructure built:**
- `scripts/setup-test-db.js` (new) — provisions `froncort_test` (idempotent), runs `prisma migrate deploy` for all 4 schemas into it, and re-applies `packages/audit-service/prisma/append-only.sql`'s exact grants against `froncort_test`'s `audit` schema (the `audit_writer` role is cluster-wide from Phase 5, but `GRANT`/`REVOKE` is per-database, so the isolated test DB needs its own). `npm run test:setup` (root).
- `jest.config.js` (new, root) — `testMatch` scoped to `tests/**/*.test.js`, `setupFiles: ['tests/helpers/testEnv.js']`.
- `tests/helpers/testEnv.js` (new) — Jest `setupFiles` entry; redirects every service's `*_DATABASE_URL` at `froncort_test` by rewriting the db name in the real `.env`'s URLs, relying on `dotenv`'s default `override: false` so each service's own `require('dotenv').config(...)` (inside its `server.js`) can't clobber the redirect when required later.
- `tests/helpers/bootstrap.js` (new) — `startStack({identity, ticket, pr, audit})`: starts exactly the services a suite needs on ephemeral ports (`.listen(0)`), wires `IDENTITY_SERVICE_URL`/`TICKET_SERVICE_URL`/`PR_SERVICE_URL`/`AUDIT_SERVICE_URL` between them (read at call-time by `identityClient`/`ticketClient`/`prClient`/`auditClient`, so setting them right before use is sufficient), connects Redis when identity-service is requested (its own `server.js` no longer does this automatically outside the `require.main` guard), and its `stop()` closes every HTTP server plus explicitly `$disconnect()`s every Prisma client used and `.quit()`s the Redis connection — explicit teardown rather than relying on Jest's `--forceExit` to paper over leaked handles.
- Root `package.json` — added `jest`, `supertest` devDependencies; `test`/`test:setup` scripts.
- **Known, accepted side effect:** `npm install` for `jest`'s dependency tree raised `npm audit`'s count from 8 to 25 findings. Confirmed via `npm audit --omit=dev` that the production/runtime dependency tree is unchanged (still exactly the same 8, already documented in `docs/known-limitations.md`) — all 17 new findings are dev-tooling-only, never shipped. Noted here for Phase 9's known-limitations pass rather than chased down now (out of this phase's "no scope creep" scope).

**3. All 5 required suites written and passing** (`npx jest --runInBand`, all 5 files together: **18/18 tests passing**, no cross-suite interference):
- **`tenant-isolation.test.js`** — two fresh orgs, a real ticket and a real PR owned by org A and never shared with org B. Org B's admin fetching/patching org A's real IDs → 404, and — the emphasized requirement — `expect(res.body).toEqual({error: {message: expect.any(String), code: 'NOT_FOUND'}})` asserted explicitly (not just the status code), plus `expect(JSON.stringify(res.body)).not.toContain('Org A only')` (the fixture's own distinguishing title) as a second, independent leak check. Also covers a fully fabricated nonexistent ID producing the identical shape.
- **`sharing.test.js`** — real connection request/approve between two fresh orgs, a real ticket share. Confirms view+comment work for the guest org, edit/delete/re-share all fail (404), then the requirement given the most explicit emphasis: revokes the underlying connection (not the share), re-fetches the share row via the real API both before and after, and asserts `expect(shareAfter).toEqual(shareBefore)` — the share row's own fields (id, createdAt, `revokedAt: null`) are provably byte-for-byte unchanged, while access is independently confirmed gone.
- **`ai-leakage.test.js`** — the one built with the most care, per the user's explicit flag. Mocks `packages/audit-service/src/lib/groqClient` and asserts on the **exact prompt string** captured from the mock's call arguments (`groqClient.generateDigest.mock.calls[0][0]`), never on stored digest output. A realistic multi-org fixture: Org A gets one fresh (non-overdue) ticket; Org B gets one ticket deliberately backdated past `TICKET_OVERDUE_THRESHOLD_DAYS` (direct Prisma write — the only thing the real API can't produce), giving Org B a real, distinguishable `overdueCount: 1` against Org A's `0`. A second fixture does the same for PR facts: one reviewer user, added as a `REVIEWER` member of **both** orgs, but only actually assigned as a PR reviewer in Org B — so the exact same person's Org A facts are `awaitingReviewCount: 0` and their Org B facts are `1`, isolating the leak boundary to org-scoping alone, independent of which user asks. Each distinguishing fact is proven real (not "absent everywhere because nothing exists") by generating that org's own digest and confirming the number DOES appear there. **Deliberate, explained deviation from the literal instruction:** "distinguishable fake data" is a distinguishable *number*, not a distinguishable ticket *title* — titles never reach `buildDigestPrompt` at all (the internal facts endpoints only ever return counts), so asserting a title string never appears in the prompt would trivially always pass regardless of whether org-scoping worked, which would be a fake test dressed as a rigorous one. A final test calls `buildDigestPrompt` directly with synthetic facts and confirms the output contains no UUIDs, emails, or org names at all — reinforcing that the function's signature itself has no parameter through which such data could leak.
- **`auth.test.js`** — see the security-sweep re-verification above; codified as two Jest scenarios (reuse-detection, and logout-everywhere) rather than one chained scenario, since both are independently session-destructive and chaining them would make the second check unable to prove anything specific.
- **`audit-permissions.test.js`** — connects as `audit_writer` directly (via `AUDIT_RUNTIME_DATABASE_URL`, redirected to `froncort_test` by `testEnv.js`) against a real seeded `AuditLog` row (inserted via `audit_writer`'s own real INSERT grant, not an owner shortcut), and asserts `$executeRawUnsafe(UPDATE ...)` / `$executeRawUnsafe(DELETE ...)` both `rejects.toThrow(/permission denied/i)`, then re-reads the row to confirm it's provably unchanged/still-present — codifying what Phase 5 and its follow-up patch had so far only proven via manual `psql` sessions.

**4. Manual 5-role spot-check against the real running app (dev DB, real ports 4001-4004), per the phase's definition of done:**
- **OA**: create ticket (201), create PR (201), manage org connections (200) — full access confirmed.
- **SA**: create ticket (201, allowed), delete ticket (403, OA-only — correctly blocked), zero pr-service create access (403). `GET /prs` returned `200 {"data":[]}`, not 403 — confirmed this is the correct, already-documented Phase 4 design (no router-level role gate on GET lists; empty-visibility enforced in the service layer), not a regression, by checking the body contains no PR data.
- **REV**: cannot create tickets or PRs (403 both), can view both lists (200). `GET /tickets`/`GET /prs` as PSA similarly returned `200 {"data":[]}` — same verified-empty-body pattern, not a leak.
- **GUEST (via share)**: Beta's OA viewing Alpha's seeded shared ticket → 200 view, 201 comment, 404 edit, 404 delete — full view+comment-only confirmed against the real app, not just the test suite.
- **PSA**: zero ticket/PR/audit-log visibility (empty lists / 403 on `/audit-log`), full org/connection management access (200) — scope matches CLAUDE.md's locked "Platform Super Admin scope" section exactly.
- Test-created rows (a handful of `title: 't'`/`description: 'd'` tickets/PRs and one guest spot-check comment, plus their audit-log entries) cleaned up afterward via direct Prisma/owner-connection access — the seeded demo data was left in the same state it started in.

**Files modified:**
- `packages/shared/corsOptions.js` (new), `packages/shared/index.js` (exports `buildCorsOptions`).
- `packages/identity-service/src/server.js`, `packages/ticket-service/src/server.js`, `packages/pr-service/src/server.js`, `packages/audit-service/src/server.js` — CORS locked down, `require.main === module` guard added, `module.exports = app`.
- `scripts/setup-test-db.js` (new).
- `jest.config.js` (new, root), `package.json` (root) — `jest`/`supertest` devDependencies, `test`/`test:setup` scripts.
- `tests/helpers/testEnv.js` (new), `tests/helpers/bootstrap.js` (new), `tests/tenant-isolation.test.js` (new), `tests/sharing.test.js` (new), `tests/ai-leakage.test.js` (new), `tests/auth.test.js` (new), `tests/audit-permissions.test.js` (new).
- `CLAUDE.md` — Commands section corrected by the user (in-session) to accurately describe the root-level `tests/` layout instead of implying per-service suites.
- Dev DB (`froncort`): a handful of manual-spot-check test rows created then deleted (tickets, one PR, one comment, matching audit-log entries) — net no change to seeded state.
- New database: `froncort_test` (isolated, disposable, never touched by manual verification).

**Remaining work:** Phase 7 (Frontend: Shared Component Library + Support Hub) is next.

**Known issues / TODOs:**
- `npm audit`'s dev-only vulnerability count rose from 8 to 25 after adding `jest`; confirmed production-tree-only count is unchanged (still 8, already in `docs/known-limitations.md`). Flag to actually add this note to `docs/known-limitations.md` at Phase 9.
- Test suites don't run in CI yet (no CI pipeline exists in this project) — `npm test` is currently a local-only, manually-invoked command requiring `npm run test:setup` to have been run at least once against a locally-running Postgres. Fine for this assignment's scope; would need a CI job (with its own Postgres/Redis services) if this were a real production repo.
- `ai-leakage.test.js`'s reviewer-fixture pattern (one user, `REVIEWER` in two orgs, real work assigned in only one) is a good template for future multi-org test scenarios but does leave that exact stray-membership shape in `froncort_test` each run — harmless since the whole database is disposable and re-seeded fresh via `npm run test:setup`, unlike the dev DB's own similar (accidental, pre-existing) stray membership noted in Phase 4/5's entries.

---

## Pre-Phase-7 checks — test idempotency, commit hygiene, stray worktree found and removed (2026-07-30)

Three things the user asked to confirm before Phase 7 code started:

**1. Commit hygiene — already clean, no action needed.** Checked `git status`/`git log` before touching anything: the "out-of-band commit" concern from the end of Phase 6 was unfounded — `84c6d14 Phase 6` already contains all 5 test files plus `docs/project-progress.md`, and the supporting test infra (`jest.config.js`, `tests/helpers/`, `packages/shared/corsOptions.js`, `scripts/setup-test-db.js`) was already committed one commit earlier in `c6c34c0 Phase 4 & 5`. Working tree had nothing pending except `.claude/` (session-local tooling config, not project code).

**2. `npm test` idempotency — confirmed true, but only after fixing a real, unrelated bug it surfaced.** `tenant-isolation.test.js`/`sharing.test.js`/`ai-leakage.test.js`/`auth.test.js` all generate fresh fixtures per run via `` `${Date.now()}-${Math.random().toString(36).slice(2)}` `` unique suffixes on every email/org name — no fixed IDs. `audit-permissions.test.js` uses `crypto.randomUUID()` for its one seeded row and explicitly deletes it in `afterAll` via the owner DB connection. Ran `npm test` twice back-to-back with no intervening `npm run test:setup` — both runs: **5/5 suites, 18/18 tests passing**, no unique-constraint or duplicate-fixture failures. Idempotency confirmed by demonstration, not just code inspection.
   - **Bug found and fixed along the way (not present before this session):** a stray git worktree at `.claude/worktrees/phase7-support-hub/` (an unrelated, unused checkout at the same commit as `main`, empty of any unique work — `git status` inside it was clean) was making Jest's Haste module map fail with "the name `@froncort/shared` was looked up in the Haste module map... several different files... provide a module for that particular name" — every test suite failed to even load `identity-service/src/server.js` because of it. Removed the worktree (`git worktree remove`, then a leftover-file `rm -rf` since a locked file blocked the first pass) and hardened `jest.config.js` with `modulePathIgnorePatterns`/`watchPathIgnorePatterns` for `<rootDir>/.claude/` so a future stray worktree there can't silently break the suite again the same way.

**3. Unrelated infrastructure incident during this session, resolved, not a code issue:** Docker Desktop's engine got stuck in a wedged "Engine stopping" state (its own API returning 500s; `docker-desktop`'s WSL2 distro showed `Stopped` while the GUI still claimed it was running) partway through Phase 7 verification, breaking Redis and causing a real `500 Internal server error` on login (`TimeoutError` on a Redis command in identity-service's logs). Fixed with the user's approval: force-killed all `Docker Desktop`/`com.docker.*` processes reachable from a non-elevated shell, `wsl --shutdown` to reset the wedged VM, relaunched Docker Desktop, `docker compose up -d` to bring `froncort-redis` back (it had exited during the reset), and restarted identity-service so it picked up a fresh Redis connection instead of the broken one. No code changes resulted from this — purely a local dev-environment hiccup, noted here only because it blocked and then explains a mid-session gap in the Phase 7 verification timeline.

**Files modified:**
- `jest.config.js` — added `modulePathIgnorePatterns`/`watchPathIgnorePatterns` for `.claude/`.
- Deleted: `.claude/worktrees/phase7-support-hub/` (stray empty worktree, unregistered from git and removed from disk).

**Remaining work:** none — Phase 7 itself follows immediately in this same session.

**Known issues / TODOs:** none new.

---

## Phase 7 — Frontend: Shared Component Library + Support Hub (2026-07-30)

**Status:** complete

**Completed features:**
- `frontend/packages/ui` — real shared component library, replacing the Phase 1 placeholders, built strictly from `reference/frontend_reference.md`'s token names (colors, radii, spacing, button/card/input/pill-tag component specs) read in full before writing any component:
  - `Button.jsx`, `Card.jsx`, `Input.jsx` (`Input`/`Select`/`Textarea`/`Label`), `Badge.jsx` (`Badge` + `TicketStatusBadge`/`TicketPriorityBadge`/`ConnectionStatusBadge`) — the design system's own "don't introduce additional accent colors as system colors" rule is why status/priority badges use only the green/soft two-tone vocabulary plus label text, not a distinct color per status.
  - `auth/apiClient.js` + `auth/AuthContext.jsx` — the real auth wiring. `apiClient.js` is a stateless fetch wrapper (holds no token itself — keeps CLAUDE.md rule #5 literally true, not just followed by convention) that does exactly one silent refresh-and-retry on any 401 before giving up. `AuthContext.jsx`'s `AuthProvider` holds the access token in React state only (never localStorage/sessionStorage), does the "silent refresh on load" via the shared httpOnly cookie (`POST /auth/refresh` with `credentials: 'include'`) in a mount effect, and decodes the JWT payload client-side (base64, unverified — display purposes only, e.g. which org is active) to expose `activeOrgId`/`orgRole`/`isPlatformAdmin` to the rest of the app. `login`/`register`/`switchOrg`/`logout` all wired to the real endpoints.
  - `AppShell.jsx`, `OrgSwitcher.jsx`, `NotificationBell.jsx`, `RoleGate.jsx` (UI-side convenience mirror of the backend's own-org+role checks — explicitly documented as NOT a security boundary, since the API enforces every check independently regardless of what renders), `CommentThread.jsx`, `TicketCard.jsx`, `PRCard.jsx`, `AuditFilterBar.jsx`, `AuditLogTable.jsx` — built now per the guide's "used by both apps, built once here even though Review Console needs it too in Phase 8" instruction, though Support Hub only wires `TicketCard`/`CommentThread` into an actual screen this phase; `PRCard`/`AuditFilterBar`/`AuditLogTable` are library-only until Phase 8 wires them into Review Console screens.
- **Real auth wiring, end-to-end, verified live in Chrome (not just code-reviewed):** login as `admin@alpha.test` → real JWT issued → dashboard renders real seeded tickets → posted a real comment (round-tripped through `POST /tickets/:id/comments`, rendered back with correct "You" attribution and timestamp) → logout → confirmed `/dashboard` redirects to `/` afterward (session actually invalidated, not just client state cleared) → confirmed direct navigation to `/dashboard` while logged out redirects to login (auth guard works, not just link-hiding).
- Support Hub screens (`frontend/apps/support-hub/app/`):
  - `page.js` — combined login/register form (toggle, not two routes), redirects to `/dashboard` once authenticated, shows a "Checking session…" state during the silent-refresh mount effect so there's no flash of the login form for an already-logged-in user.
  - `dashboard/layout.js` — auth guard (redirects to `/` if not authenticated) + `AppShell` wrapper with Tickets/Connections/Feature Flags nav.
  - `dashboard/page.js` — ticket list with status filter (re-fetches on filter change AND on org-switcher change — verified the org switcher isn't just a label, it drives a real refetch), inline "New Ticket" create form gated to OA/SA via `RoleGate`.
  - `dashboard/tickets/[id]/page.js` — ticket detail: status/priority/assignedTo editable (OA/SA of the ticket's own org only — gated on BOTH role AND `ticket.orgId === activeOrgId`, matching the backend's "never via a share" rule, not just role), attachments (upload + list, OA/SA own-org only), delete (OA own-org only), and the comment thread (any caller who could view the ticket can also comment, matching `api_reference.md`'s identical population for both).
  - `dashboard/connections/page.js` — cross-org connection request/approve/revoke, scoped to the org switcher's current org; explicitly does NOT attempt a "PSA manages any org's connections" console (PSA has no `activeOrgId`/`OrgMembership` by design, so "my org's connections" isn't a concept that applies to them, and that's not in Phase 7's documented scope).
  - `dashboard/flags/page.js` — read-only feature flag list.
- `next.config.js` (support-hub) — loads the repo-root `.env` via `dotenv` and re-exposes the 4 `NEXT_PUBLIC_*_API_URL` vars (already documented in `.env.example` since Phase 1, just never consumed until now) through Next's `env` config key, since Next.js only auto-discovers `.env*` files inside the app's own directory, not the monorepo root every backend service already shares.

**Verification:**
- Live Chrome walkthrough (not just curl) covering: login, ticket list with real seeded data (status/priority badges rendering correctly), ticket detail (status/priority selects, assignedTo field, attachments section, comment post-and-render round trip), org switcher (dropdown opens, shows Alice's one real membership), connections page (real seeded connection rows with Approve/Revoke), feature flags page (real `AI_DIGEST_ENABLED` flag), logout, and the post-logout auth-guard redirect.
- Backend regression check: re-ran the full Jest suite twice after all frontend changes (see the pre-Phase-7 entry above) — 18/18 both times, confirming nothing in Phase 7 touched backend behavior.

**Files modified:**
- `frontend/packages/ui/` — `Button.jsx`, `Card.jsx` rewritten; new: `Input.jsx`, `Badge.jsx`, `RoleGate.jsx`, `AppShell.jsx`, `OrgSwitcher.jsx`, `NotificationBell.jsx`, `CommentThread.jsx`, `TicketCard.jsx`, `PRCard.jsx`, `AuditFilterBar.jsx`, `AuditLogTable.jsx`, `auth/apiClient.js`, `auth/AuthContext.jsx`, `index.js` (barrel updated).
- `frontend/apps/support-hub/` — `next.config.js` (dotenv + env passthrough), `package.json` (added `dotenv` dependency), `app/layout.js` (wraps `AuthProvider`), `app/page.js` (real login/register), `app/dashboard/layout.js` (new, auth guard + `AppShell`), `app/dashboard/page.js` (new, ticket list), `app/dashboard/tickets/[id]/page.js` (new, ticket detail), `app/dashboard/connections/page.js` (new), `app/dashboard/flags/page.js` (new).

**Remaining work:** Phase 8 (Review Console) is next — reuses this phase's `PRCard`/`AuditFilterBar`/`AuditLogTable`/`AppShell`/`AuthProvider` rather than rebuilding any of it.

**Known issues / TODOs — real frontend/backend contract gaps found and flagged, not silently smoothed over, per the explicit instruction to flag these rather than improvise:**
- **No endpoint anywhere resolves a `userId` to a display name.** `Comment.authorId`, `Attachment.uploadedBy`, `Ticket.assignedTo`/`createdBy` are all bare UUIDs — ticket-service has no cross-schema join to identity-service's `User` table (by design, per `CLAUDE.md`'s "no cross-schema FKs"), and no route in `api_reference.md` exposes a name-lookup for a caller who isn't that org's admin (the closest thing, `GET /internal/users/:userId/org-role`, is internal-API-key-gated, service-to-service only). Handled pragmatically rather than inventing a new endpoint (CLAUDE.md rule #11): `CommentThread` shows "You" for the current user's own comments and `Member <first 8 chars of UUID>` otherwise; the ticket detail page's read-only `assignedTo` fallback does the same. This is a real, load-bearing UX gap, not a cosmetic one — worth raising with whoever owns `api_reference.md` if a future phase wants real names shown.
- **No endpoint lists an org's current members**, only `POST`/`PATCH`/`DELETE /orgs/:id/members` (add/update-role/remove) — there's no `GET`. This isn't a Phase 7 gap exactly (member management isn't in Phase 7's documented scope, only "org connections management UI" is), but it does mean the ticket create/assign form's `assignedTo` field is necessarily a free-text UUID input with a helper label ("UUID of an existing org member"), not a dropdown — there's no data source to populate one from. Flagging in case a later phase's spec assumes a member picker exists somewhere.
- **No org directory endpoint** — the connections page's "request a new connection" form is also necessarily a free-text target-org-ID input, with an explanatory note that the ID has to come from the other org's admin out-of-band. Not treated as a bug (nothing in `api_reference.md` suggests a directory should exist), just noted as a UX ceiling given the current contract.
- **Mobile-responsive polish was explicitly deprioritized per the user's own stated ordering** ("if time gets tight, mobile-responsive polish is the first thing to cut, not auth correctness or role-gating") — Tailwind's responsive utilities are used in a few places (ticket grid `sm:`/`lg:` column counts) but the app has not been tested at a narrow viewport width. Everything else in the stated priority order (auth wiring + org switcher, ticket list/detail with role-gating, connections, feature flags) was completed and verified live.
- PSA has no dedicated connections-management view in Support Hub (see `dashboard/connections/page.js`'s header comment) — PSA's own org/connection administration is really identity-service's own domain, not something this ticketing-app UI was ever meant to surface; flagging only so it's not mistaken for an oversight later.
