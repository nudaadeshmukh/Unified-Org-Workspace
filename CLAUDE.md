...
9. **Every mutation that's a "reportable action" per the spec calls `auditClient.log(...)` and blocks on it — synchronously, and *before* the actual database mutation runs, not after.** If the audit call fails or times out, abort the whole operation with an error and perform no database write at all; never let a mutation succeed with no corresponding audit entry. This is achievable cleanly because resource IDs are Prisma-generated client-side (see Conventions) — for a create, the ID exists before any DB write, so it can be included in the audit call up front. For updates/deletes, the ID already exists. **This finally resolves the swallow-and-warn stopgap used in Phases 2–4 before audit-service existed** — that stopgap is now retired; every service's `logAudit()` must be updated to this blocking, audit-first ordering as part of Phase 5. If you add a new mutation and you're unsure whether it needs an audit entry, check `api_reference.md`'s audit-actions list before deciding it doesn't.
...
- `npm test` (root) — runs the cross-service integration suites in `tests/` against an isolated `froncort_test` database (never the seeded `froncort` dev database). All 5 required suites (tenant-isolation, sharing, ai-leakage, auth, audit-permissions) are inherently cross-service and live here, not under any individual service's package — a service package only gets its own `npm test` in the future if a genuinely service-local unit test is ever added, which none currently are.
