...
- **Decision update, superseding the custom-domain requirement below:** after weighing the time cost against the deliverable requirement, the decision was made to deploy on Railway's default assigned domains (no custom domain purchased). This trades away full cross-dashboard session sync (documented as a known limitation, not hidden) in exchange for actually shipping the "Hosted Project Public URL" deliverable rather than skipping it. Two things this requires, both already decided and not open for re-discussion:
  1. **`SameSite=None` for the refresh cookie in production** (configurable via `COOKIE_SAME_SITE` env var, default `lax` for local dev — do not change local behavior). Without this fix, login would fail in 100% of browsers once deployed, since frontend and identity-service will be on different registrable domains under Railway's default domains, making the session cookie's request genuinely cross-site — `SameSite=Lax` is never sent on cross-site `fetch`/XHR calls.
  2. **`docs/known-limitations.md` must clearly document two things**, worded as engineering trade-offs, not failures: (a) cross-dashboard session sync isn't demonstrable on this hosted deployment without a shared parent domain, though it's implemented and verified via automated tests and local development; (b) the resulting cross-site session cookie is a third-party cookie from the browser's perspective, which Safari and Firefox block by default — state plainly what was actually observed when testing the deployed URL in those browsers, not an assumption.
- ~~Custom domain required before deployment~~ — **superseded, see above.** (Original reasoning retained for context: session sync depends on both dashboards sharing one parent domain for the refresh cookie; default `*.vercel.app`/`*.up.railway.app` subdomains belong to different root domains and cannot share a cookie the way a real owned domain's subdomains can.)
- Deploy: Railway for the 4 backend services + Postgres + Redis; Railway (or Vercel) for both Next.js apps, deployed independently, on their default assigned domains. Set `CORS_ALLOWED_ORIGINS` and `NEXT_PUBLIC_*_API_URL` to the real deployed URLs once known (requires one deploy to learn the URLs, then a config update + redeploy).
